package {
	...
	public class NewgroundsAPI {
		...
		public static function linkAPI(movie:MovieClip):void 
		{
			movie.addChild(bridge);
			linked = true;
		}

		public static function connectMovie(id:Number):void 
		{
			if (!id) {
		...
		public static function setMovieVersion(movie_version:Number):void {
			if (!movie_version) {
		...
		public static function addEventListener(event:Number, callback:Function):void 
		{
			events[event] = callback;
		}
		
		public static function addCustomEvent(stat_id:Number, stat_name:String):void 
		{
			if (!stat_id) {
		...
		public static function addCustomLink(stat_id:Number, stat_name:String):void 
		{
			if (!stat_id) {
		...
		public static function loadMySite(event:Event = null):void 
		{
			SendLink(STAT_AUTHOR_SITE);
		}
		
		public static function loadNewgrounds(event:Event = null, page:String = null):void 
		{
			if (!tracker_id) {
		...
		public static function logCustomEvent(event_name:String):void 
		{
			if (!event_name) {
		...
		public static function loadCustomLink(link_name:String):void 
		{
			if (!link_name) {
		...
		private static function SendEvent(id:Number):void 
		{
			SendStat(id,false);
		}
		
		private static function SendLink(id:Number, extra:Object = null):void 
		{
			SendStat(id,true,extra);
		}
		
		private static function ReadGatewayData(params:Object):void 
		{
			...
			if (params['settings_loaded']) {
				...
				if (movie_options['request_portal_url']) {
					...
					var target_url:String = GATEWAY_URL+"?&id="+tracker_id+"&portal_url="+escape(_url);
		...
		private static function SendStat(stat_id:Number, open_in_browser:Boolean, extra:Object = null):void 
		{
			if (!tracker_id) {
				SendError('You must call NewgroundsAPI.connectMovie() with a valid movie id before using API features!');
			} else {
				var target_url:String = GATEWAY_URL+"?&id="+tracker_id+"&host="+escape(host)+"&stat="+stat_id+addSeed();

				if (extra) {
					for(var x:String in extra) {
				...
				function XML_Loaded(event:Event):void {
					...
					for each (var XML_child:XML in XML_children) {
						...
						var param_name:Object = XML_child.localName();
						var param_value:Number = XML_child.attribute('value');
		...
		private static function SendMessage(msg:String):void 
		{
			trace("[NEWGROUNDS API] :: "+msg);
		}
		
		public static function onNewVersionAvailable(version:String, movie_url:String, redirect_url:String):void 
		{
			...
			var _root:MovieClip = MovieClip(bridge.root);
			...
			overlay.close = function(event:Event):void {
		...
		public static function onDenyHost(hostname:String, movie_url:String, redirect_url:String):void 
		{
			var sw:int = bridge.stage.stageWidth;
			var sh:int = bridge.stage.stageHeight;
			var tw:int = 350;
		...
		public static function onAdsApproved(ad_url:String):void { }
